...
function AgentList () {
  ...
  const handleOpenModal = async (agentId) => {
    ...
    try {
      const response = await axios.get(`https://8080-deefcbababbddcfcdcebdafbcfcbaedbffbeeaadbbb.project.examly.io/agents/properties/${agentId}`, {
        headers: {
  ...
  const handleDelete = async (agentId) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`https://8080-deefcbababbddcfcdcebdafbcfcbaedbffbeeaadbbb.project.examly.io/agents/${agentId}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        });
  ...
  useEffect(() => {
    const fetchAgents = async () => {
      try {
        const response = await axios.get('https://8080-deefcbababbddcfcdcebdafbcfcbaedbffbeeaadbbb.project.examly.io/agents', {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        });
