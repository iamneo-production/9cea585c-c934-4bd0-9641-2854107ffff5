...
const UserList = () => {
  ...
  const handleDelete = async (userId) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`https://8080-feaaeedcbbebeeddcfcdcebdafbeaeaadbdbabf.project.examly.io/users/${userId}`, {
          headers: {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get('https://8080-feaaeedcbbebeeddcfcdcebdafbeaeaadbdbabf.project.examly.io/users', {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
        });
