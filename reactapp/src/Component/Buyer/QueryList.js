...
const QueryList = () => {
  ...
  const fetchQueries = useCallback(async () => {
    try {
      if (userId && userRole) {
        const response = await axios.get(`https://8080-eddfcabaeaccfeddcfcdcebdafbcfcbaedbffbeeaadbbb.project.examly.io/Query/${userId}/${userRole}`, {
          headers: {
  ...
  const postReply = async (queryId) => {
    try {
      ...
      await axios.post(`https://8080-eddfcabaeaccfeddcfcdcebdafbcfcbaedbffbeeaadbbb.project.examly.io/Query/${queryId}`, `${userRole}: ${replyText}`, {
        headers: {
