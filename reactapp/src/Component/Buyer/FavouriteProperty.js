...
function FavouriteProperty() {
  ...
  const handleFavouriteClick = async (propertyId) => {
    try {
      ...
      if (favourite) {
        await axios.delete(`https://8080-feaaeedcbbebeeddcfcdcebdafbeaeaadbdbabf.project.examly.io/favourites?favId=${favourite.id}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
        });
        ...
        const response = await axios.post("https://8080-feaaeedcbbebeeddcfcdcebdafbeaeaadbdbabf.project.examly.io/favourites", {
          userId: userId,
