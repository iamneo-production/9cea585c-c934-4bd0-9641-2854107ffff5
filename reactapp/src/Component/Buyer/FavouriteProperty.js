...
function FavouriteProperty() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("https://8080-deefcbababbddcfcdcebdafbeaeaadbdbabf.project.examly.io/properties", {
          headers: {
      ...
      try {
        if (userRole === "buyer") {
          const response = await axios.get(
            `https://8080-deefcbababbddcfcdcebdafbeaeaadbdbabf.project.examly.io/favourites/user?userId=${userId}`, {
            headers: {
              Authorization: `Bearer ${localStorage.getItem('token')}`
            }
          });
  ...
  const handleFavouriteClick = async (propertyId) => {
    try {
      ...
      if (favourite) {
        await axios.delete(`https://8080-deefcbababbddcfcdcebdafbeaeaadbdbabf.project.examly.io/favourites?favId=${favourite.id}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
        });
        ...
        const response = await axios.post("https://8080-deefcbababbddcfcdcebdafbeaeaadbdbabf.project.examly.io/favourites", {
          userId: userId,
