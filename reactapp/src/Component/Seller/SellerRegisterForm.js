...
function SellerRegisterForm({ onCloseModal }) {
  const initialState = {
    name: '',
    email: '',
    phone: '',
  ...
  const handleFileChange = (event) => {
    ...
    setValidationErrors({ ...validationErrors, profileImageError: '' });
    // Validate profile image size
    if (file && file.size > 1024 * 1024) {
      setValidationErrors({ ...validationErrors, profileImageError: 'Profile image should be less than 1MB.' });
      event.target.value = "";
    }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
<<<<<<< HEAD
      const response = await axios.post('https://8080-facbdebeebddcfcdcebdafbcfcbaedbffbeeaadbbb.project.examly.io/agents', formData, {
=======
      const response = await axios.post('https://8080-feaaeedcbbebeeddcfcdcebdafbcfcbaedbffbeeaadbbb.project.examly.io/agents', formData, {
  ...
  return (
    <Container>
      <Form onSubmit={handleSubmit}>
        ...
        <Form.Group className="mb-3">
          <Form.Label>
            <AiFillFileImage /> Profile Image *
          </Form.Label>
          <Form.Control type="file" name="profileImage" onChange={handleFileChange} />
          <Form.Text className="text-muted">{validationErrors.profileImageError}</Form.Text>
        </Form.Group>
