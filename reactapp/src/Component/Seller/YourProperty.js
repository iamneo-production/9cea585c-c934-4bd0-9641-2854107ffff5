...
function YourProperty() {
  ...
  const fetchUserProperties = async () => {
    try {
      ...
      const response = await axios.get(`https://8080-aacbbdbdbffeddcfcdcebdafbcfcbaedbffbeeaadbbb.project.examly.io/agents/properties/${userId}`, {
        headers: {
  ...
  const handleDelete = async (propertyId) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`https://8080-aacbbdbdbffeddcfcdcebdafbcfcbaedbffbeeaadbbb.project.examly.io/properties/${propertyId}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
        });
